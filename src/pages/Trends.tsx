...
export function Trends({ storage }: Props) {
  ...
  function buildPath(metricKey: Metric): string {
    ...
    const points = melderReports.map((r, i) => {
      ...
      const y = padding.top + innerH - ((val / 150) * innerH);
      return `${x},${y}`;
    });
  ...
  function buildDots(metricKey: Metric) {
    return melderReports.map((r, i) => {
      const x =
        padding.left +
        (melderReports.length > 1 ? (i / (melderReports.length - 1)) * innerW : innerW / 2);
      const val = Math.min(getValue(r, metricKey), 150);
      const y = padding.top + innerH - ((val / 150) * innerH);
      ...
      {melder && melderReports.length === 0 && (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-100">
          <p className="text-slate-400 text-sm">
            No reports for {melder.name} yet. Create one in the Calculator.
          </p>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          {momentum && (
            <div className="grid grid-cols-3 gap-4">
              {(
                [
                  { key: 'oap', label: 'OAP', delta: momentum.oap, color: '#6366f1' },
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">vs {momentum.prevPeriod}</p>
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-slate-400">
                  {roleName} · {melderReports.length} month
                  {melderReports.length !== 1 ? 's' : ''} of data
                </p>
              </div>
              ...
              <tbody>
                {[...melderReports].reverse().map((r) => (
                  <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                    ...
                    <td className="px-5 py-3">
                      <HealthBadge
                        health={r.ratioResult.health}
                        label={fmtPct(r.ratioResult.ratio)}
                        size="sm"
                      />
                    </td>
                    <td className="px-5 py-3">
