    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <div className="mt-6 bg-slate-50 rounded-2xl p-5 border border-slate-100">
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-black text-indigo-600">{storage.melders.length}</p>
            <p className="text-xs text-slate-500">Melders</p>
          </div>
          <div>
            <p className="text-2xl font-black text-indigo-600">{storage.reports.length}</p>
            <p className="text-xs text-slate-500">Reports</p>
          </div>
          <div>
            <p className="text-2xl font-black text-indigo-600">{storage.roles.filter((r) => r.isCustom).length}</p>
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
      ...
      <button
        onClick={onClick}
        disabled={disabled}
        className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-xl transition-colors disabled:opacity-40 disabled:cursor-not-allowed bg-indigo-600 text-white hover:bg-indigo-700"
      >
